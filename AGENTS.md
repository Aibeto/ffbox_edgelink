...
- 运行时分层：`main.js`（宿主，常驻：设 XDG_CONFIG_HOME/TMPDIR → 监听控制 Unix socket `filesDir/nodejs-project/nodectl.sock` → worker_threads 拉起/终止 `index.cjs`）。**引擎只启动一次且常驻**（nodejs-mobile 限制：Node 线程不可重启、process.exit 会杀整个 app 进程）；FFBox 后端跑在 worker 中。**停止语义（勿改回 taskPauseBatch）**：parentPort 收 stop → 先 `storeUnfinishedTask` 快照 + 等 900ms（越过 700ms 防抖写盘）→ `taskResetBatch` 软停止全部活跃任务（向 ffmpeg 发 'q' 优雅收尾，**不可用 taskPauseBatch**——SIGSTOP 挂起的进程在 worker 退出后成孤儿，持有内存与文件句柄）→ 轮询等 stopping→idle（5s 上限）→ 超时对残余 `forceKill` → `process.exit(0)`（worker 内仅结束线程，端口自动释放）→ 可再次拉起。退出须赶在 reset 回调触发的空快照防抖（700ms）写盘前，否则 `lastStatus.tasks` 被清空、任务无法恢复。
